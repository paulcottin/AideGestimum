...
public class Utils extends Observable implements LongTask {
	...
	public void supprImagesNonUtilisees(ArrayList<File> files) throws IOException {
		ArrayList<File> imgDisk = new ArrayList<File>();
		ArrayList<String> imgProjet = new ArrayList<String>();
		ArrayList<File> htmlFile = new ArrayList<File>();
		...
		for (File file : files) 
			if (file.getAbsolutePath().endsWith(".png") || file.getAbsolutePath().endsWith(".jpg") ||
				...
				imgDisk.add(file);
			else if (file.getAbsolutePath().endsWith(".htm") || file.getAbsolutePath().endsWith(".htt"))
				htmlFile.add(file);
		...
		for (File file : htmlFile) {
			...
			for (org.jsoup.nodes.Element element : img) {
				if (!imgProjet.contains(getAbsolutePath(file, element.attr("src")))) {
					imgProjet.add(getAbsolutePath(file, element.attr("src")));
				}
			}
		}
		int cpt = 0;
		for (File file : imgDisk) {
			if (!imgProjet.contains(file.getAbsolutePath())) {
				file.delete();
				cpt++;
			}
		}
		Principale.messageFin(cpt+" images inutilisées supprimées dans le projet");
		//System.out.println("disk : "+imgDisk.size()+", projet : "+imgProjet.size()+", delete : "+cpt+", delta : "+(imgDisk.size()-imgProjet.size()-cpt));
	}
	
	private String getAbsolutePath(File file, String relativePath) {
		//Détermination du nombre de retour arrière
		String[] tab = relativePath.split("\\.\\.");
		int nbRetours = tab.length-1;
		String filePath = file.getAbsolutePath();
		tab = filePath.split("\\\\");
		String absolute = "";
		for (int i = 0; i < tab.length-1-nbRetours; i++) {
			absolute += tab[i]+"\\";
		}
		String newRelativePath = relativePath.substring(nbRetours*3, relativePath.length()).replace("/", "\\");
		return absolute+newRelativePath;
	}
	
	public void helpCopieEcran(ArrayList<File> files) throws ParametrageError {
		...
		new VisualisationImg(imgFile, 0);
	}
	...
	public int indexInArray(ArrayList<File> fileList, String name) {
